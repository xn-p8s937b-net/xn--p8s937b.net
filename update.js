...
const keysToPunycode = (records) => Object.fromEntries(Object.entries(records)
    .map(([key, value]) => [punycode.toASCII(key), value]))

async function main() {
    const cf = cloudflare({
        token: CF_TOKEN,
    })
    const local = keysToPunycode(JSON.parse(await fs.readFile('records.json')))
    ...
    const remote = keysToPunycode(Object.fromEntries(filtered.map((record) => [record.name.split('.')[0], record.content])))
